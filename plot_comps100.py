...
pth = os.path
debug = False
def edit_plotscript(folder, nstart=1, nend=100):
	...
	with open("vis_hdf5.m", 'r') as f:
		for i, line in enumerate(f.readlines()):
			...
			if i == 29:
				...
				sp_var1 = splitline[2]
				line = line.replace(sp_var1, "%d;" % 20)
			elif i == 41:
				sp_var2 = splitline[2]
				line = line.replace(sp_var2, "%d;" % 0)
			elif i == 42:
				sp_var3 = splitline[2]
				line = line.replace(sp_var3, "%d;" %0)				
			elif i == 52:
				zoom = splitline[2]
...
def main():
	...
	folders = str('ER, ES, ET, EU, EV, EX, EY, EZ, FA, FB, FC, FD, FE, FF, FG, FH, FI, FJ, FK, FL, FM, FN, FO, FP, FQ, FR, FS, FT, FU').split(', ')
