	...
	with open("vis_hdf5.m", 'r') as f:
		for i, line in enumerate(f.readlines()):
			...
			if i == 29:
				...
				sp_var1 = splitline[2]
				line = line.replace(sp_var1, "%d;" % 8) # These and the next two ifs refer to the variable ids defined in (py)vis_hdf5.m so now viscosity, normal stress and total strain rate is shown.  
			elif i == 41:
				sp_var2 = splitline[2]
				...
				zoom = splitline[2]
				line = line.replace(zoom, "%d;" %1) # 0 = not zoomed, 1 = zoomed. You have to change the zoom inside vis_hdf5.m though. 
